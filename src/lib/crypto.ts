...
async function getKey(salt: Buffer): Promise<Buffer> {
  ...
  const derived = await new Promise<Buffer>((resolve, reject) => {
    crypto.pbkdf2(key, salt, KEY_ITERATIONS, KEY_LENGTH, "sha512", (err, derivedKey) => {
      if (err) reject(err);
...
export async function encryptMessage(text: string): Promise<string> {
  ...
  const cipher = crypto.createCipheriv(ALGORITHM, await getKey(salt), iv);
...
export async function decryptMessage(encryptedText: string): Promise<string> {
  try {
    ...
    const decipher = crypto.createDecipheriv(ALGORITHM, await getKey(salt), iv);
