...
export default function GenerateSubjectsPage() {
  ...
  const [level, setLevel] = useState(isAdmin ? "LEVEL_1" : userLevel);
  ...
  }, [logPage, logSearch, logFilter, userRole]);
  ...
  const handleGenerate = async () => {
    if (!subjectName.trim()) {
      showToast("الرجاء إدخال اسم المادة", "warning");
      return;
    }
    setGenerating(true);
    setProgress(0);
  ...
  const generateTextFile = (entry: any) => {
    const lines = [
      ...
      `الترم: ${entry.semester === "TERM_1" ? "الترم الأول" : "الترم الثاني"}`,
      `كود تفعيل المعلم: ${entry.teacherCode}`,
      "",
    ];
  ...
  const handleDownloadSingle = (entry: LogEntry) => {
    ...
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `code-${entry.subjectName || entry.name}.txt`;
    a.click();
  };
  ...
  const toggleSelectAll = () => {
    if (selectedIds.size === logs.filter((l) => l.role === "TEACHER").length)
      setSelectedIds(new Set());
    else
      setSelectedIds(
        new Set(logs.filter((l) => l.role === "TEACHER").map((l) => l.id)),
      );
  };
      ...
      <div
        ...
        >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "15px" }}>
              ...
              <div>
                ...
                >
                  {isAdmin
                    ? "الأدمن"
                    : `الإدارة - ${userLevel === "LEVEL_1" ? "المستوى الأول" : "المستوى الثاني"}`}
                </p>
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            <motion.button
              ...
              style={{
                ...
                background: generating
                  ? "#1a1a2e"
                  : "linear-gradient(135deg,#bf5af2,#a855f7)",
                color: "#fff",
                fontWeight: 800,
                fontSize: "1rem",
                cursor: "pointer",
            ...
            >
              {generating ? "⏳ جاري التوليد..." : "🚀 توليد المادة الدراسية"}
            </motion.button>
          </motion.div>
          ...
          >
            ...
            >
              ...
              <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                ...
                <select
                  ...
                  style={{ ...inputStyle, width: "130px", padding: "8px" }}
                >
            ...
            {loadingLogs ? (
            ...
            ) : logs.filter((l) => l.role === "TEACHER").length === 0 ? (
              <div
                style={{
                  textAlign: "center",
                  padding: "40px",
              ...
              <>
                ...
                >
                  {selectedIds.size ===
                  logs.filter((l) => l.role === "TEACHER").length
                    ? "❌ إلغاء الكل"
                    : "✅ تحديد الكل"}
                </button>
                      ...
                      {logs
                        ...
                        .map((entry, i) => (
                          ...
                          >
                            ...
                            >
                              {entry.semester === "TERM_1" ? "ترم1" : "ترم2"}
                            </td>
                            <td
                              style={{
                                padding: "10px",
