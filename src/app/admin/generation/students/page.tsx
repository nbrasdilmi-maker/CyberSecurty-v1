...
export default function GenerateStudentsPage() {
  ...
  const handleGenerate = async () => {
    const namesList = names
      ...
      .filter(Boolean);
    if (!namesList.length) {
      showToast("الرجاء إدخال اسم واحد على الأقل", "error");
      return;
    }
    setGenerating(true);
    setProgress(0);
  ...
  const generateTextFile = (entries: any[]) => {
    const lines = [
      ...
      "أكواد تفعيل حسابات الطلاب",
      "=".repeat(60),
      "",
      `التاريخ: ${new Date().toLocaleDateString("ar-YE")}`,
      `المستوى: ${level === "LEVEL_1" ? "المستوى الأول" : "المستوى الثاني"}`,
      `عدد الحسابات: ${entries.filter((e: any) => e.success).length}`,
      "",
    ];
    entries
      .filter((e: any) => e.success)
      .forEach((e: any) => {
        lines.push(`الاسم: ${e.name}`, `كود التفعيل: ${e.code}`, "");
      });
      ...
      <PageTransition>
        ...
        >
          ...
          >
            ...
            >
              {generating ? "⏳ جاري التوليد..." : "🚀 توليد الحسابات"}
            </motion.button>
          </motion.div>
