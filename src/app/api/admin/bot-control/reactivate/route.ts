...
export const POST = withErrorHandler(async function POST(request: NextRequest) {
  ...
  await tigService.removePermanentLock(binding.telegramId);
  ...
  await prisma.auditLog.create({
    data: {
      ...
      action: "UPDATE",
      severity: "INFO",
      description: "تم إعادة تفعيل البوت للمستخدم لإعادة الربط",
      ipAddress: ip,
      level: null,
    },
  });

  return NextResponse.json({ success: true, message: "تم إعادة التفعيل. يمكن للمستخدم ربط البوت مرة أخرى." });
});
